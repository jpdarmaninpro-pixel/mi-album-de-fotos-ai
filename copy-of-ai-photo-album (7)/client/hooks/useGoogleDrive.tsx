                ...
                callback: async (tokenResponse: any) => {
                    ...
                    if (tokenResponse.access_token) {
                      ...
                      try {
                        ...
                        // **CRITICAL FIX**: Add timeout to root folder creation/retrieval
                        const timeoutPromise = new Promise<never>((_, reject) =>
                          setTimeout(() => reject(new Error("Operation timed out.")), 15000)
                        );
                        
                        const folderId = await Promise.race([
                            findOrCreateRootFolder(window.gapi),
                            timeoutPromise
                        ]);
                        setRootFolderId(folderId);

                      } catch (error) {
                        console.error('Error fetching user profile or finding root folder:', error);
                        const errorMessage = error instanceof Error ? error.message : String(error);
                        setInitError(`Failed to prepare your Google Drive folder after sign-in. This could be a network issue or a problem with Google Drive permissions. Please try refreshing the page. Details: ${errorMessage}`);
                      }
                    }
                },
            });
